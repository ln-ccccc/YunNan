...
class AttentionFusion(nn.Module):
    def __init__(self, in_channels_x1, in_channels_x2, attn_channels=24):
        super().__init__()
        self.project_x2 = None
        if in_channels_x2 != in_channels_x1:
            self.project_x2 = nn.Conv2d(in_channels_x2, in_channels_x1, kernel_size=1)
            attn_in = in_channels_x1 + in_channels_x1
        else:
            attn_in = in_channels_x1 + in_channels_x2
        self.attn = nn.Sequential(
            nn.Conv2d(attn_in, attn_channels, kernel_size=1),
            nn.ReLU(),
    ...
    def forward(self, x1, x2):
        x2p = x2 if self.project_x2 is None else self.project_x2(x2)
        attn_weight = self.attn(torch.cat([x1, x2p], dim=1))
        return attn_weight * x1 + (1 - attn_weight) * x2p
...
class DINOv3SwinEncoder(BaseModule):
    def __init__(self,
        ...
        if self.use_cross_stage:
            ...
            self.intra_fusion_layers = self._build_intra_fusion_layers(
                fusion_cfg, self.intra_stage_fusion_type, feature_adapt_cfg)
    ...
    def _build_adapt_layers(self, feature_adapt_cfg, dinov3_out_channels, swin_embed_dims):
        if feature_adapt_cfg is None:
            layers = nn.ModuleList()
            for _ in range(len(swin_embed_dims)):
    ...
    def _build_intra_fusion_layers(self, fusion_cfg, intra_type, feature_adapt_cfg):
        intra_layers = nn.ModuleList()
        norm_cfg = fusion_cfg.get('norm_cfg')
        for i, swin_ch in enumerate(self.swin_embed_dims):
            # 确定输入融合层的DINOv3特征通道数
            # 如果没有适配层(A1消融实验)，则是原始DINOv3通道(1024)
            # 如果有适配层(正常情况)，则是适配后的Swin通道(128等)
            if feature_adapt_cfg is None:
                dinov3_input_ch = self.dinov3_out_channels[i]
            else:
                dinov3_input_ch = swin_ch

            if intra_type == 'attention':
                intra_layers.append(
                    AttentionFusion(
                        in_channels_x1=swin_ch,
                        in_channels_x2=dinov3_input_ch,
                        attn_channels=fusion_cfg.get('attn_channels', 24)
                    )
                )
            elif intra_type == 'gated':
                # GatedFusion通常假设通道一致，如果不一致可能需要修改GatedFusion实现
                # 此处暂时保持原样，假设GatedFusion在A1中不被使用或已处理
                 intra_layers.append(
                    GatedFusion(
                        in_channels=swin_ch,
                        norm_cfg=norm_cfg
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            # 阶段内融合
            if self.use_cross_stage:
                # 修改：交换参数顺序，确保x1对应Swin通道(128)，x2对应DINOv3通道(1024或128)
                # AttentionFusion初始化时设定x1=swin_ch, x2=dinov3_input_ch
                # 这样AttentionFusion内部的project_x2可以将DINOv3特征投影到Swin维度
                intra_fused = self.intra_fusion_layers[i](swin_feat, dinov3_adapted)
            else:
