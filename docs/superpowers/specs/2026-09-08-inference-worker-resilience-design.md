...
> 状态：已实施（2026-09-08）。实施与验证提交：`71d11e6`（worker 进程内任务隔离 + 回归测试 + stale mock 修复）、`5c41dbd`（KML-ROI 逐阶段计时）。验证记录：容器内单文件 26/26 → 推理族 85/85 → 全量 discover 302 例仅剩 2026-09-07 台账中的已知环境项；真实连续双跑（预载模型 + 真实流水线，同一 worker 连续两个任务）均成功且 stage_durations 输出真实数据。
