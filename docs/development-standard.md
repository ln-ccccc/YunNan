...
每项任务写一个主模块负责人和一个复核人，角色可以轮换。例如甲负责项目/空间/成果后端，乙负责 Miner/GeoView；推理和部署单独排任务，不要求两人同时跨所有模块。
...
另一个开发者复核调用边界、主要 diff 和测试证据，至少重走一条用户路径。Agent 可以协助独立审查，人负责确认验收含义；不能仅凭 Agent 的“全部通过”合入。
