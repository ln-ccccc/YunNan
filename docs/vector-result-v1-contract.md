...
> 状态：待后端负责人起草、GeoView 前端负责人评审后冻结。本文冻结的是计划中的 v1 数据与 HTTP 边界；截至本文创建时，它们不是对既有路由已经实现的声明。
...
- 当前仓库只有 `admin_user_id` 登录态，尚无项目成员、角色或多用户项目权限模型。待实现的 v1 只能沿用这一现状：已登录管理员可在系统内操作项目；本文不把它描述成成员隔离或只读/编辑角色控制。
...
下表是待实现的 v1 路由声明，不表示当前仓库已存在同名 Blueprint 或处理器。
...
待实现的 API 测试必须分别覆盖：超过 10 MiB、超过 2,000 个要素、单要素超过 5,000 个顶点、总顶点超过 100,000 的四个边界；每一例均断言 HTTP `422`、无部分保存、无新修订、无审计记录、当前版本不变。测试还必须覆盖 NaN/Infinity、超经纬度、非面/无效面、ROI 外面、跨成果或跨基准版本 `feature_id`，以及 `vector_failed` 的 `409` 响应。
